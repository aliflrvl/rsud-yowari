...
elseif ($_GET[module]=='fasilitas'){
  $sql  = mysql_query("SELECT * FROM modul WHERE id_modul='27'");
  $r    = mysql_fetch_array($sql);

  echo "<h2>Fasilitas Pelayanan</h2>
        <form method=POST enctype='multipart/form-data' action=aksi.php?module=fasilitas&act=update>
        <input type=hidden name=id value=$r[id_modul]>
        <table>
        <tr><td><img src=foto_berita/$r[gambar]></td></tr>
        <tr><td>Ganti Foto : <input type=file size=30 name=fupload></td></tr>
...
elseif ($_GET[module]=='standar'){
  $sql  = mysql_query("SELECT * FROM modul WHERE id_modul='28'");
  $r    = mysql_fetch_array($sql);

  echo "<h2>Standar Pelayanan</h2>
        <form method=POST enctype='multipart/form-data' action=aksi.php?module=standar&act=update>
        <input type=hidden name=id value=$r[id_modul]>
        <table>
        <tr><td><img src=foto_berita/$r[gambar]></td></tr>
        <tr><td>Ganti Foto : <input type=file size=30 name=fupload></td></tr>
...
elseif ($_GET[module]=='tempattidur'){
  include "modul/mod_tempattidur.php";
}

// Bagian Video
elseif ($_GET[module]=='video'){
  include "modul/mod_video.php";
}
